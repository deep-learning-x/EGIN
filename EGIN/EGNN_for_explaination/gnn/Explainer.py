...
class Explainer(torch.nn.Module):
    ...
    def explain_graph(self, graph):
        ...
        for epoch in range(1, self.epochs + 1):
            optimizer.zero_grad()
            graph.x = (graph.x * self.node_feat_mask.view(1, -1).sigmoid())
            out = self.model(graph)
            log_logits = self.__to_log_prob__(out)
    ...
    def visualize_subgraph(self, node_idx, edge_index, edge_mask, y=None,
        ...
        label_kwargs['font_size'] = kwargs.get('font_size') or 11
        ...
        if node_alpha is None:
            ...
            nx.draw_networkx_nodes(G, pos, alpha=node_alpha_subset.tolist(),node_size=400,
                                   node_color=y.tolist(), linewidths=None, **node_kwargs)
